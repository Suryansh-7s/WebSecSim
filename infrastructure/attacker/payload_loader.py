import yaml
import os
import sys

# Path to where the repo is cloned
ART_PATH = "/opt/atomic-red-team/atomics"

def get_atomic_command(t_code):
    # DIRECT MAPPING: T1087.001 input -> Look for T1087.001 folder
    yaml_path = os.path.join(ART_PATH, t_code, f"{t_code}.yaml")
    
    if not os.path.exists(yaml_path):
        return None, f"YAML file not found at {yaml_path}"

    try:
        with open(yaml_path, 'r') as f:
            data = yaml.safe_load(f)
            
        for test in data.get('atomic_tests', []):
            if 'linux' in test.get('supported_platforms', []):
                command = test['executor']['command']
                name = test['name']
                return command, name
                
        return None, "No Linux test found for this T-Code"

    except Exception as e:
        return None, str(e)

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 payload_loader.py <T_CODE>")
        sys.exit(1)

    t_code = sys.argv[1]
    cmd, name = get_atomic_command(t_code)
    
    if cmd:
        print(f"---FOUND: {name}---")
        print(cmd)
    else:
        print(f"ERROR: {name}")
        sys.exit(1)